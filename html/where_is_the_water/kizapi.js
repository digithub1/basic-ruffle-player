...
var _KIZ10API = function () {
  ...
  this.set_GAMEID = function (VALUE) {
    this._GAMEID = VALUE;
  };
  this.get_APIKEY = function () {
    return this._APIKEY;
  };
  ...
  this._VALUES;
  this._SETARRAY_STATS = function (STATS) {
    this._VALUES = [STATS];
  };
  this.get_VALUEAPI = function (COD_STAT) {
    return this._VALUES[COD_STAT];
  };
  this.set_VALUEAPI = function (COD_STAT, VALUE) {
    this._VALUES[COD_STAT] = VALUE;
  };
  this._phpApi = "_apiGames.php";
  this._USERID;
          ...
          if (ajax.readyState == 4) {
            ...
            if (typeof _response != "undefined" && _response != "") {
              ...
              if (jsonObj.success && jsonObj.data.action == "SUCCESS") {
                var STAT_VALUE;
                if (typeof jsonObj.stat.value != "undefined") {
                  STAT_VALUE = jsonObj.stat.value;
                  this.set_VALUEAPI(COD_STAT, STAT_VALUE);
                }
                var STAT_ACTION;
                if (typeof jsonObj.stat.action != "undefined") {
                  STAT_ACTION = jsonObj.stat.action;
                }
                var STAT_CODE;
                if (typeof jsonObj.stat.code != "undefined") {
                  STAT_CODE = jsonObj.stat.code;
                }
                _updateValues();
              }
            }
  ...
  this.set_STATVALUE = function (COD_STAT, VALUE) {
    console.log(
      "Submitstat : " + COD_STAT + " - " + VALUE + " (" + isNaN(VALUE) + ")"
    );
    if (isNaN(VALUE) == true) {
      console.log("NAME VAR " + VALUE);
              ...
              if (jsonObj.success && jsonObj.data.action == "SUCCESS") {
                ...
                if (typeof c != "undefined" && c != "") {
                  for (var j = 0; j < c; j++) {
                    ...
                    if (
                      _action == "JUMP ACHIEVEMENT" &&
                      typeof _code != "undefined" &&
                      _code != ""
                    ) {
                      _jumpAchiev = 1;
                      achievs += _code + ",";
                    ...
                    console.log(
                      jsonObj.achievement[j].code +
                        "::" +
                        jsonObj.achievement[j].status +
                        "::" +
                        jsonObj.achievement[j].progress +
                        "::" +
                        jsonObj.achievement[j].goal +
                        "::" +
                        jsonObj.achievement[j].action
                    );
                  }
                }
                ...
                if (
                  _statAction == "INSERT NEW VALUE STAT" ||
                  _statAction == "UPDATE VALUE STAT" ||
                  _jumpAchiev == 1
                ) {
                  _callAPIcheckScores(achievs);
                }
              }
            }
          }
  ...
  this.unlock_ACHIEVEMENT = function (ACHIEV, PROGRESS) {
    if (
      ...
      if (GAME_ID != "" && API_KEY != "") {
        var PARAMS =
          "ACTION=UNLOCK_ACHIEVEMENT&USER_ID=" +
          USER_ID +
          "&ID_GAME=" +
          this._GAMEID +
          "&API_KEY=" +
